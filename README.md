...
All Python scripts are run through the [BlueWaters bwpy-environ environment](https://bluewaters.ncsa.illinois.edu/python). Before running any python scripts, you must first execute:
...
to initialize the environment. This only has to be done once per Bash session. Then, any Python scripts can be run as normal.
